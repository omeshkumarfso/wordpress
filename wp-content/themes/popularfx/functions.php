...
if ( ! defined( 'POPULARFX_VERSION' ) ) {
	...
	define( 'POPULARFX_VERSION', '1.1.4' );
}
...
function popularfx_promos(){
	...
	if(is_admin() && current_user_can('install_themes')){
		...
		popularfx_maybe_promo([
			'after' => 1,// In days
			'interval' => 30,// In days
			'pro_url' => esc_url(POPULARFX_PRO_URL),
			'rating' => esc_url('https://wordpress.org/themes/popularfx/#reviews'),
			'twitter' => esc_url('https://twitter.com/PopularFXthemes?status=').rawurlencode('I love #PopularFX Theme by @pagelayer team for my #WordPress site - '.esc_url(home_url())),
			'facebook' => esc_url('https://facebook.com/popularfx'),
			'website' => esc_url(POPULARFX_WWW_URL),
...
function popularfx_scripts() {
	...
	if(!empty($template)){
		...
		wp_enqueue_style( 'popularfx-sidebar', get_stylesheet_directory_uri().'/sidebar.css', array(), POPULARFX_VERSION );
	}else{
		wp_enqueue_style( 'popularfx-style', get_stylesheet_uri(), array(), POPULARFX_VERSION );
		wp_style_add_data( 'popularfx-style', 'rtl', 'replace' );
	}	

	wp_enqueue_script( 'popularfx-navigation', get_stylesheet_directory_uri() . '/js/navigation.js', array(), POPULARFX_VERSION, true );
...
<script type="application/javascript">
	...
	<div class="'.(empty($opts['class']) ? 'notice notice-success' : 'popularfx-templates-promo').'" id="popularfx_templates_promo" style="min-height:90px">
		<div class="popularfx_promo-left">
			<img src="'.POPULARFX_URL.'/images/templates.png" width="85%" />
		</div>
		...
		<a class="popularfx_promo-close" href="javascript:" aria-label="Dismiss this Notice">
			<span class="dashicons dashicons-dismiss"></span> Dismiss
				...
				<br>
				'.(empty($opts['pro_url']) || !empty($popularfx['license']) ? '' : '<a class="button button-primary" target="_blank" href="'.$opts['pro_url'].'">'.__('Buy PopularFX Pro', 'popularfx').'</a>').'
				'.(empty($opts['rating']) ? '' : '<a class="button button-primary" target="_blank" href="'.$opts['rating'].'">'.__('Rate it 5★\'s', 'popularfx').'</a>').'
				'.(empty($opts['facebook']) ? '' : '<a class="button button-secondary" target="_blank" href="'.$opts['facebook'].'"><span class="dashicons dashicons-thumbs-up" style="vertical-align: middle;"></span> '.__('Facebook', 'popularfx').'</a>').'
				'.(empty($opts['twitter']) ? '' : '<a class="button button-secondary" target="_blank" href="'.$opts['twitter'].'"><span class="dashicons dashicons-twitter" style="vertical-align: middle;"></span> '.__('Tweet', 'popularfx').'</a>').'
				'.(empty($opts['website']) ? '' : '<a class="button button-secondary" target="_blank" href="'.$opts['website'].'">'.__('Visit our website', 'popularfx').'</a>').'
			</p>
		</div>
	</div>';
...
<script type="application/javascript">
	...
	if(!empty($opts['image'])){
		echo '<a href="'.$opts['website'].'"><img src="'.$opts['image'].'" style="float:left; margin:25px 20px 10px 10px" width="67" /></a>';
	}
	...
	echo '
		...
		'.(empty($opts['pro_url']) || !empty($popularfx['license']) ? '' : '<p style="font-size:13px">'.sprintf( __('%1$s has many more features like 500+ Website %2$s Templates%3$s, 90+ widgets, 500+ sections, Theme Builder, WooCommerce Builder, Theme Creator and Exporter, Form Builder, Popup Builder, etc. You get a Pagelayer Pro license with the PopularFX Pro version.', 'popularfx'), '<a href="'.$opts['pro_url'].'"><b>PopularFX Pro</b></a>', '<a href="'.$opts['website'].'/templates">', '</a>').'
		</p>').'
		</div>';
	...
	echo '<div class="popularfx_promo-right-1">
			...
			<center style="margin:10px;">
				<a href="'.esc_url(POPULARFX_WWW_URL.'/templates').'" style="text-decoration: none; font-size: 15px;"><img src="'.POPULARFX_URL.'/images/templates.png" width="100%" /><br><br>'.__('Install from 500+ Website Templates', 'popularfx').'</a>
			</center>
		</div>
	</div>';
...
function popularfx_maybe_promo($opts){
	...
	//echo $opt_name.' - '.$func.' - '.date('Ymd', $promo_time).'<br>';
